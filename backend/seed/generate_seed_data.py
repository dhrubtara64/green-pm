...
Project: Northgate Combined Cycle Gas Turbine (CCGT) Power Plant — 450MW
...
Demo scenarios engineered per Product Bible / diagram 17:
  A — Drawing approval: 3 corroborating sources, clean history → Ev≈0.85, Conf≈0.85
  B — Vendor delivery: 3 sources agree, BAD vendor history → Ev≈0.80, Conf≈0.35 [KEY DIVERGENCE]
  C — Site excavation: 1 stale source → Ev≈0.20, Conf≈0.30 [LOW-LOW]
  D — Inspection: 1 fresh highly-reliable source → Ev≈0.30, Conf≈0.75 [LOW-EVIDENCE, HIGH-CONF]
  E — Cabling: contradicting evidence → Verification Required
...
# Allow imports from the backend app
...
def seed(session):
    print("Seeding Northgate CCGT Power Plant project...")
    ...
    # --- SCENARIO A: act-010 — GT Receipt & Inspection (Evidence 0.85, Confidence 0.85) ---
    # 3 corroborating sources, clean history, all recent, all high reliability
    ev_a1 = EvidenceItem(
        id="ev-a1",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="GT_Receipt_Inspection_Report_2024-11-15.pdf",
        extracted_content="Gas Turbine Model GE 9HA.01 received at Northgate site. Serial No. 7A4129. "
                          "Visual inspection completed. All major components accounted for. No transit damage observed. "
                          "Signed: Site Inspector J. Chambers, QA Lead M. Patel. Date: 15 Nov 2024.",
    ...
    ev_a2 = EvidenceItem(
        ...
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Transmittal_GE_9HA01_Delivery_Confirmation.pdf",
        extracted_content="Delivery Transmittal No. TRS-2024-447. GE Gas Turbine Package confirmed delivered "
                          "to Northgate Power Station. 100% of line items confirmed received and checked against "
                          "packing list. Transmittal accepted by Client Representative.",
    ...
    ev_a3 = EvidenceItem(
        ...
        provenance_ref="Northgate_CCGT_P6_Export_2024-11-20.xer",
    ...
    # --- SCENARIO B: act-013 — HRSG Drum Installation (Evidence 0.80, Confidence 0.35) ---
    # 3 sources agree on 60% but vendor (Thermax) has a history of overclaiming
    ev_b1 = EvidenceItem(
        id="ev-b1",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Thermax_Progress_Report_Nov_2024.pdf",
        extracted_content="Thermax Industries Progress Report — November 2024. HRSG Drum and Header "
                          "installation: 60% complete. Drums 1 and 2 positioned. Header connections in progress. "
                          "Note: Previous month's report claimed 55% — actual site measurement shows 60%.",
        source_excerpt="Thermax self-report: HRSG drum installation 60% complete. Nov 2024.",
        relation_type=RelationType.supports_progress_of,
        source_reliability_signal=SourceReliabilitySignal.low,  # vendor self-reporting with bad history
        timestamp=ago(5),
    )
    ev_b2 = EvidenceItem(
        id="ev-b2",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Thermax_Site_Photo_Report_2024-11-18.pdf",
        extracted_content="Site photo documentation — HRSG installation area. Photos show Drum 1 and Drum 2 "
                          "in position on HRSG structure. Steam headers partially connected. Photo timestamp "
                          "18-Nov-2024. Estimated progress: 58-62%.",
        source_excerpt="Site photos confirm HRSG drums positioned, headers partial. ~60% progress. 18 Nov 2024.",
        relation_type=RelationType.supports_progress_of,
        source_reliability_signal=SourceReliabilitySignal.low,  # photos from same vendor
        timestamp=ago(9),
    )
    ev_b3 = EvidenceItem(
        id="ev-b3",
        ...
        extracted_content="TASK TSK-013: HRSG Drum and Header Installation. Physical % Complete: 60%. "
                          "Note: Progress entered by Thermax site representative. Not independently verified this period.",
        source_excerpt="P6: TSK-013 at 60%. Input from Thermax — not independently verified.",
        relation_type=RelationType.supports_progress_of,
        source_reliability_signal=SourceReliabilitySignal.medium,
        timestamp=ago(7),
    )
    ...
    # Single stale source, no independent corroboration
    ev_c1 = EvidenceItem(
        id="ev-c1",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Site_Weekly_Update_2024-10-04.txt",
        extracted_content="Weekly site update 4-Oct-2024: GT foundation concrete pour in progress. "
                          ...
                          "Source: Site foreman verbal update, logged by Site Engineer.",
        source_excerpt="GT foundation concrete pour ~85%. Foreman verbal update, 4 Oct 2024. Unverified.",
        relation_type=RelationType.supports_progress_of,
        source_reliability_signal=SourceReliabilitySignal.low,
        timestamp=ago(28),  # stale — 4 weeks old
    )
    evidence_items.append(ev_c1)

    # --- SCENARIO D: act-022 — HP Steam Piping Hydrostatic Test (Evidence 0.30, Confidence 0.75) ---
    # Single source but fresh and highly reliable (certified test record)
    ev_d1 = EvidenceItem(
        id="ev-d1",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="HP_Steam_Hydro_Test_Witness_Record_2024-11-22.pdf",
        extracted_content="HYDROSTATIC TEST WITNESS RECORD. Line: HP Steam Main — N'Gate-HS-001. "
                          "Test Pressure: 145 bar. Hold time: 1 hour. Result: PASS. No leaks detected. "
                          "Witnessed by: Third Party Inspector (Bureau Veritas) — Cert No. BV-2024-NT-0471. "
                          "Client Representative: R. Singh, PE. Date: 22 November 2024.",
        source_excerpt="HP Steam hydro test PASSED. Witnessed by Bureau Veritas BV-2024-NT-0471. 22 Nov 2024.",
        relation_type=RelationType.supports_progress_of,
        source_reliability_signal=SourceReliabilitySignal.high,  # third-party certified
        timestamp=ago(3),  # very fresh
    )
    evidence_items.append(ev_d1)

    # --- SCENARIO E: act-032 — MCC Cabling (Verification Required — contradiction) ---
    # Two contradicting sources at same time
    ev_e1 = EvidenceItem(
        id="ev-e1",
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Electrical_Weekly_Report_2024-11-15.pdf",
        extracted_content="MCC Room cabling installation status: 55% complete. Cable tray installation "
                          ...
                          "Motor control cables in progress. Sub-contractor Voltline Electrical Pvt Ltd.",
    ...
    ev_e2 = EvidenceItem(
        ...
        source_system="document_folder",
        ingesting_connector="document_folder_ingestor_v1",
        provenance_ref="Site_QC_Inspection_MCC_2024-11-18.pdf",
        extracted_content="QC INSPECTION REPORT — MCC Room Cable Installation. Date: 18-Nov-2024. "
                          ...
                          "as claimed in progress report. Actual measured completion: 30-35%. "
                          "NCR raised: NCR-2024-E-041. Previous progress claim of 55% cannot be substantiated.",
        source_excerpt="QC Inspection 18 Nov 2024: MCC cabling actually 30-35%, not 55%. NCR-2024-E-041 raised.",
        relation_type=RelationType.contradicts,  # CONTRADICTION
        source_reliability_signal=SourceReliabilitySignal.high,
        timestamp=ago(8),
    )
    evidence_items.extend([ev_e1, ev_e2])

    # --- Additional evidence for other activities (realistic corpus) ---
    ev_misc = [
        # act-001: site preparation — complete, well documented
        EvidenceItem(
            id="ev-m1", source_system="schedule_xer",
            ingesting_connector="schedule_xer_ingestor_v1",
            provenance_ref="Northgate_CCGT_P6_Export_2024-11-20.xer",
            extracted_content="TSK-001: Site Preparation and Clearing. Status: Complete. Actual Finish: 15-Mar-2024. 100%.",
            source_excerpt="P6: Site preparation 100% complete, Mar 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.medium,
            timestamp=ago(7),
        ),
        # act-004: HRSG steelwork — partial evidence
        EvidenceItem(
            id="ev-m2", source_system="document_folder",
            ingesting_connector="document_folder_ingestor_v1",
            provenance_ref="HRSG_Steel_Progress_2024-11-10.pdf",
            extracted_content="HRSG Structural Steelwork Bay A: Column erection complete. Primary beam connections "
                              "70% complete. Secondary steelwork ongoing. Estimated completion end of November 2024.",
            source_excerpt="HRSG Bay A steelwork 70% complete. Primary beams done. Nov 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.medium,
            timestamp=ago(17),
        ),
        # act-030: Main transformer — high confidence
        EvidenceItem(
            id="ev-m3", source_system="document_folder",
            ingesting_connector="document_folder_ingestor_v1",
            provenance_ref="Main_Transformer_FAT_Certificate_2024-10-30.pdf",
            extracted_content="FACTORY ACCEPTANCE TEST CERTIFICATE — Main Power Transformer 450MVA 400/20kV. "
                              "Manufacturer: Siemens Energy. All factory tests passed. Certificate No. SE-2024-FAT-0872. "
                              "Delivered to Northgate site 05-Nov-2024.",
            source_excerpt="Main transformer FAT passed. Siemens SE-2024-FAT-0872. Delivered 5 Nov 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.high,
            timestamp=ago(22),
        ),
        EvidenceItem(
            ...
            ingesting_connector="schedule_xer_ingestor_v1",
            provenance_ref="Northgate_CCGT_P6_Export_2024-11-20.xer",
            extracted_content="TSK-030: Main Transformer Receipt and Inspection. Status: Complete. Actual Finish: 07-Nov-2024.",
            source_excerpt="P6: Main transformer receipt 100% complete 7 Nov 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.medium,
            timestamp=ago(7),
        ),
        # act-040: DCS — in progress
        EvidenceItem(
            id="ev-m5", source_system="document_folder",
            ingesting_connector="document_folder_ingestor_v1",
            provenance_ref="DCS_Installation_Status_2024-11-14.pdf",
            extracted_content="DCS Cabinet Installation — Northgate CCGT. 18 of 24 cabinets fully wired. "
                              "Remaining 6 cabinets in progress. FAT certificates received for DCS hardware. "
                              "Estimated completion: 10-Dec-2024.",
            source_excerpt="DCS: 18/24 cabinets wired (60%). Remaining 6 in progress. Nov 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.medium,
            timestamp=ago(13),
        ),
        # act-050: pre-commissioning civil — well supported
        EvidenceItem(
            id="ev-m6", source_system="document_folder",
            ingesting_connector="document_folder_ingestor_v1",
            provenance_ref="Pre_Comm_Civil_Checklist_2024-11-20.pdf",
            extracted_content="Pre-Commissioning Checklist — Civil and Structural. Total items: 45. "
                              "Completed: 36. Outstanding: 9 items (minor punchlist). Estimated 80% complete. "
                              "Signed off by: Site Manager K. Olawale, PE.",
            source_excerpt="Pre-comm civil 36/45 items complete (80%). Signed K. Olawale PE. 20 Nov 2024.",
            relation_type=RelationType.supports_progress_of,
            source_reliability_signal=SourceReliabilitySignal.high,
            timestamp=ago(7),
        ),
        ...
        EvidenceItem(
            ...
            provenance_ref="Northgate_CCGT_BOQ_Rev5.xlsx:row_42",
            extracted_content="BOQ Item 4.3: MCC Room Cable Installation | Qty: 1 Lot | Rate: £285,000 | "
                              "Claimed to date: £156,750 (55%) | Approved to date: £95,000 (33%).",
            source_excerpt="BOQ: MCC cabling 55% claimed, only 33% approved. Discrepancy flagged.",
    ...
    for link in links:
        session.execute(
            activity_evidence_items.insert().values(**link)
        )
    ...
    # -------------------------------------------------------------------------
    # COMPUTE SCORES — using deterministic formula (LLM part seeded with pre-set values)
    # -------------------------------------------------------------------------
    _seed_scores(session)

    print(f"  {len(all_activities)} activities seeded")
    ...
    print("Done. Run recompute-all via the API to refresh LLM-based Confidence Scores.")


def _seed_scores(session):
    """
    Pre-seed scores to match the engineered demo scenarios.
    The live /ingest/recompute-all endpoint will recompute these from actual evidence.

    Scenario A: GT Receipt — Evidence 0.850, Confidence 0.850
    Scenario B: HRSG Drum — Evidence 0.800, Confidence 0.350 [KEY DIVERGENCE]
    Scenario C: GT Foundation — Evidence 0.200, Confidence 0.300
    Scenario D: HP Hydro Test — Evidence 0.300, Confidence 0.750
    Scenario E: MCC Cabling — Verification Required
    """
    from app.agents.confidence_agent import compute_evidence_score

    activity_score_overrides = {
        "act-010": {"ev": 0.850, "conf": 0.850,
                    "reasoning": "Three independent sources (official delivery inspection, transmittal record, and schedule) all corroborate 100% completion. Source reliability is high. Confidence closely matches Evidence Score.",
                    "missing": None, "vr": False},
        "act-013": {"ev": 0.800, "conf": 0.350,
                    "reasoning": "Three sources agree on 60% progress, but all originate from Thermax Industries — the subcontractor with a documented history of overclaiming physical completion. The schedule entry is also Thermax-sourced. Independent verification is absent. Confidence is significantly lower than the Evidence Score warrants.",
                    "missing": "Independent third-party site inspection or Client Representative walkdown to verify Thermax's 60% claim",
                    "vr": False},
        "act-003": {"ev": 0.200, "conf": 0.300,
                    "reasoning": "Only one source — a four-week-old informal foreman verbal update. No drawing, inspection record, or schedule confirmation. Too stale and too informal to trust at face value.",
                    "missing": "Recent site inspection record or concrete pour completion certificate (last update is 28 days old)",
                    "vr": False},
        "act-022": {"ev": 0.300, "conf": 0.750,
                    "reasoning": "A single source, but it is a Bureau Veritas third-party witnessed hydrostatic test certificate — the highest-reliability source type for this activity. One authoritative certified record outweighs multiple informal updates.",
                    "missing": None, "vr": False},
        "act-032": {"ev": 0.500, "conf": 0.250,
                    "reasoning": "QC Inspection report (NCR-2024-E-041) directly contradicts the subcontractor progress claim. BOQ approval rate also contradicts the 55% claim. Verified completion is 30-35%, not 55%.",
                    "missing": "Resolution of NCR-2024-E-041 and independent re-measurement of MCC cabling completion",
                    "vr": True},
    }
    ...
    # Compute deterministic evidence scores for all other activities
    ...
    for activity in all_activities:
        if activity.id not in activity_score_overrides:
            ...
            # Simple heuristic confidence for activities without LLM scoring
